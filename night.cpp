...
// 1. iris 4 mẫu 150 example 

// 3. Sửa lại cách lưu node-> leaf_value thành vector<Ciphertext> leaf_value_vector để lưu trữ trọng số rơi vào từng lớp

// 4. Bỏ rotate_vector 

//5. Split data bỏ random -> Tự sửa dữ liệu ở file.csv 

//6. Thêm hàm hiển thị cây trước và sau giải mã print_tree_decrypted

// acc ~0.3 nhưng dự đoán full vào 1 label ? 

//7. Chọn ngưỡng khác [-0.2 , 0.2] if(val >= -0.2 && val <= 0.2) continue; 

// NOTE 
// evaluator.mod_switch_to_inplace(result, powers.back().parms_id()); co the bo 

// theta 4 , depth 2 
// --- F1 Score Per Class ---
// Class | Precision | Recall | F1-Score
// ------|-----------|--------|-----------
//     0 |    0.8182 | 0.900000 | 0.857143
//     1 |    1.0000 | 1.000000 | 1.000000
//     2 |    0.8889 | 0.800000 | 0.842105
...
string src_model_tree = "C:/hu/decision-tree-he/model_tree/model_tree_night_2.bin"; // C:\hu\decision-tree-he\Build_Project\model_tree_sevent.bin
...
vector<int> coeff_modulus_bits = 
        {60, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40} ; // 17x40  
...
double all_vector_in_index_zero(const vector<double> &vec, int number){
    double sum = 0.0;
    for(int i = 0 ; i < number ; i++){
        sum += vec[i];
    }
...
struct Node_c {
    ...
    vector<Ciphertext> feature_index; // k gia tri voi 1 gia tri 1, k-1 gia tri 0 
...
// sum_slots(ct) : tong tat ca cac slot trong ciphertext ct
...
// leaf_value(W, Y) = sum(W * Y) cho tat ca cac nhan
// Output: vector<Ciphertext> C_leaf_values co kich thuoc num_label, moi phan tu la tong W*Y cho tung nhan, chi lay o slot 0
vector<Ciphertext> leaf_value(
    ...
    for (size_t l = 0; l < num_label ; ++l) {
        ...
        Ciphertext sum_P = sum_slots(P, NUM_SAMPLES_TRAIN, evaluator, gal_keys);
        C_leaf_values[l] = sum_P;
    }
    cout << "Completed leaf_value() = " << endl;
...
pair<vector<Ciphertext>, vector<Ciphertext>> compute_weighted_counts_homo(
    vector<Ciphertext> best_feature, // [Tinh X_col] 1 số plaintext 
...
pair<Ciphertext, Ciphertext> compute_W_phi_best(
    vector<Ciphertext> best_feature, //one-hot 
...
unique_ptr<Node_c> train_decision_tree(
    ...
    const SEALContext& context,
    int num_feature, 
    vector<vector<Ciphertext>> C_I_one_hot
){
    ...
    for(int i=0 ; i<num_feature ; ++i){
        ...
        vector<Ciphertext> C_one_hot_feature = C_I_one_hot[i];
    ...
    for(int i=0 ; i<num_feature ; ++i){
        ...
        for(int j=0 ; j<num_theta ; ++j){
            ...
            for (size_t l = 0; l < num_label; ++l) {
                ...
                // right_counts_clear[l] = all_vector_in_index_zero(decoded_r, num_feature);
                // left_counts_clear[l] = all_vector_in_index_zero(decoded_l, num_feature);
    ...
    vector<Ciphertext> C_best_feature = C_I_one_hot[best_feature];
...
void save_ciphertext_vector(
    const vector<Ciphertext> &vec,
    size_t sz, // kich thuoc vector 
    ostream &os
) {
    os.write((char*)&sz, sizeof(sz));

    cout << "save vector<Ciphertext>, size=" << sz << endl;
...
vector<Ciphertext> load_ciphertext_vector(
    istream &is,
    // size_t sz, // kich thuoc vector 
    const SEALContext &context
) {
    ...
    cout << "load vector<Ciphertext>, size=" << sz << endl;

    vector<Ciphertext> vec(sz);
    for (size_t i = 0; i < sz; ++i) {
        vec[i].load(context, is);
    }
    return vec;
}

void save_node(const unique_ptr<Node_c> &node, ostream &os, int num_feature)
{
    ...
    {
        // chỉ save leaf_value
        save_ciphertext_vector(node->leaf_value_vector, num_label, os);
    ...
    // node split
    save_ciphertext_vector(node->feature_index, num_feature, os);
    ...
    if (has_left) save_node(node->left_child, os, num_feature);
    if (has_right) save_node(node->right_child, os, num_feature);
}

unique_ptr<Node_c> load_node(istream &is, const SEALContext &context, int num_feature)
{
    cout << "load_node()" << endl;
    bool exist;
    ...
    {
        cout << "is_leaf" << endl;
        node->leaf_value_vector = load_ciphertext_vector(is, context);
    ...
    node->feature_index= load_ciphertext_vector(is, context);
    ...
    if (has_left) node->left_child = load_node(is, context, num_feature);
    if (has_right) node->right_child = load_node(is, context, num_feature);

    return node;
}

void save_model(const unique_ptr<Node_c> &root, const string &filepath, int num_feature)
{
    cout << "save_model()" << endl;
    ofstream ofs(filepath, ios::binary);
    if (!ofs) throw runtime_error("Cannot open model file to save!");
    save_node(root, ofs, num_feature);
    cout << "completed save_model()" << endl;
}

unique_ptr<Node_c> load_model(const string &filepath, const SEALContext &context, int num_feature)
{
    cout << "load_model()" << endl;
    ifstream ifs(filepath, ios::binary);
    if (!ifs) throw runtime_error("Cannot open model file to load!");
    return load_node(ifs, context, num_feature);
}
...
)
{
    ...
    vector<Ciphertext> i_best = node_c->feature_index;
...
void print_tree_decrypted(
    ...
    if (node_c->is_leaf) {
        ...
        for (size_t i = 0; i < node_c->leaf_value_vector.size(); ++i) {
            Plaintext pt;
            decryptor.decrypt(node_c->leaf_value_vector[i], pt);
    ...
    int feature_index = -1;
    for (size_t i = 0; i < node_c->feature_index.size(); ++i) {
        Plaintext pt;
        decryptor.decrypt(node_c->feature_index[i], pt);

        vector<double> vals;
        encoder.decode(pt, vals);

        if (vals[0] > 0.5) { // one-hot
            feature_index = static_cast<int>(i);
            break;
        }
    }
...
int main(){
    try{
        ...
        cout << "5.1 MA HOA TUNG FEATURE DANG vector<Ciphertext> DE PHUC VU CHO TINH TOAN." << endl;
        // mã hóa số 0
        Plaintext p_0;
        ...
        for (size_t i = 0; i < NUM_SAMPLES_TEST; ++i) {
            ...
            double score_virginica = decoded_predictions[2][i]; // Lấy giá trị cột tiếp theo
            ...
            cout << setw(3) << i << " | " 
                ...
                << setw(15) << fixed << setprecision(5) << score_virginica << " | "
