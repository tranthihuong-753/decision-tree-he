    ...
    const RelinKeys& relin_keys, double scale, const SEALContext& context)
{
    ...
    try {
        size_t NUM_SAMPLES = C_X_cols.size();
        // Tạo one-hot dưới dạng Plaintext nhưng chỉ encode cho best_feature
        vector<double> v(NUM_SAMPLES, 0.0);
        if (best_feature >= 0 && best_feature < (int)NUM_SAMPLES) v[best_feature] = 1.0;
        Plaintext pt_onehot;
