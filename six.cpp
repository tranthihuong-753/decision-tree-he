// 1.Chuyển qua img chuẩn 
// path_preprocessed_csv = 'C:/hu/decision-tree-he/Build_Project/Release/cats_vs_dogs_dataset_resnet.csv'
// Kích thước vector đặc trưng ResNet50 (sử dụng GAP): (2048,) (2048 features)
// Đã xử lý 21616 ảnh.
// Đang sử dụng path_preprocessed_csv = 'C:/hu/decision-tree-he/Build_Project/Release/cats_vs_dogs_dataset_resnet_mini.csv' 
// 3 img cat, 3 img dog để test code 

// 2. Do dữ liệu khác nên sẽ sửa map<string, int> label_map

// 3. Thay vì hard code NUM_SAMPLES_TRAIN, NUM_SAMPLES_TEST thì đọc tính từ ALL_SAMPLES

// 4. train 4 test 2 (2048 pixel) time = 48h chưa xong nên chuyển qua sử dụng dataset nhỏ hơn 
// cats_vs_dogs_dataset_resnet_mini.csv  

// 5. train 4 test 2 (100 pixel) time = (2 p.m -  p.m) cats_vs_dogs_dataset_resnet_mini_100.csv 
...
// Dữ liệu cần hard code trước khi chạy 
int number_threshold = 2; // Số ngưỡng để thử nghiệm trong train_decision_tree() thực tế số ngưỡng N x k = 70 x 4 = 280 
int max_depth = 4;
int ALL_SAMPLES = 6; // tổng số mẫu trong dữ liệu
int NUM_SAMPLES_TRAIN = ALL_SAMPLES*0.8; 
int NUM_SAMPLES_TEST = ALL_SAMPLES - NUM_SAMPLES_TRAIN;
int NUM_FEATURE = 2048;
...
) ///?
{
    ...
    for(size_t i = 1; i < SOFT_STEP_COEFFICIENTS.size(); ++i) { // i < SOFT_STEP_COEFFICIENTS_16.size()
        ...
        Ciphertext term;
        cout << "  Multiplying coeff with z^" << endl;
...
Ciphertext compute_C_X_i(
    ...
    cout << " num_features=" << C_X_cols.size() << endl;
    int num_features = C_X_cols.size();

    vector<double> one_hot_best_feature(C_X_cols.size(), 0.0);
    one_hot_best_feature[best_feature] = 1.0;
    Plaintext pt_one_hot;
    encoder.encode(one_hot_best_feature, scale, pt_one_hot);
    Ciphertext C_one_hot_best_feature;
    encryptor.encrypt(pt_one_hot, C_one_hot_best_feature);
...
// trong pipeline phải chuyển best_feature thành ciphertext one-hot 
    ...
    const RelinKeys& relin_keys, double scale, const SEALContext& context, const GaloisKeys& gal_keys)
{
    ...
    Ciphertext C_X_i = compute_C_X_i(
        best_feature, C_X_cols, scale, encryptor, encoder, evaluator, gal_keys, relin_keys
    );
    ...
    // Mã hóa Ngưỡng Theta
    Plaintext pt_theta;
    encoder.encode(best_threshold, target_scale, pt_theta);
    Ciphertext C_Theta;
    encryptor.encrypt(pt_theta, C_Theta);
...
)
{
    ...
    if (node->is_leaf) {
        ...
        auto w_data = context.get_context_data(C_W_current.parms_id());
        size_t w_level = w_data->chain_index();
        double w_scale = C_W_current.scale();

        for (size_t l = 0; l < NUM_LABELS; ++l) {
            // 1) Broadcast giá trị leaf_value[l] thành plaintext có slot_count phần tử
            vector<double> leaf_expanded(NUM_SAMPLES_TEST);
            for (size_t s = 0; s < NUM_SAMPLES_TEST; ++s) leaf_expanded[s] = node->leaf_value[l];

            Plaintext pt_leaf_expanded;
            // Mã hóa với cùng scale như W để giảm việc phải thay đổi scale nhiều
            encoder.encode(leaf_expanded, w_scale, pt_leaf_expanded);

            // 2) Mod-switch plaintext xuống cùng parms_id với C_W_current
            evaluator.mod_switch_to_inplace(pt_leaf_expanded, C_W_current.parms_id());

            // 3) Multiply_plain: C_W_current * pt_leaf_expanded
            Ciphertext C_Leaf_l;
            evaluator.multiply_plain(C_W_current, pt_leaf_expanded, C_Leaf_l);
...
)
{
    ...
    for (size_t i = 0; i < NUM_SAMPLES_TEST; ++i)
    {
        ...
        cout << " So sanh voi nhan that Sample " << i << endl;
        int pred_label = 0;
...
int main()
{
    ...
    //     root, C_X_cols_test, evaluator, encryptor, encoder, 
