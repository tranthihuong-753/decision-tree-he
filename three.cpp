// soft-step-binary.cpp
// Sửa lại hàm SplitData split_data_random() để chia tập dữ liệu với số mẫu động
// Sửa lại for khi hiển thị dự đoán trong main()
...
string src_data = "C:/hu/decision-tree-he/Build_Project/Release/iris.csv";
...
// ///123 
//         Plaintext pt_leaf;
//         encoder.encode(node->leaf_value, scale, pt_leaf);

//         Ciphertext C_Leaf_Output;
//         encryptor.encrypt(pt_leaf, C_Leaf_Output);
...
//         node->right_child, C_X_cols, evaluator, encryptor, encoder, 
//         relin_keys, context, scale
//     );

//     Ciphertext C_Output_Left = predict_decision_tree(
//         node->left_child, C_X_cols, evaluator, encryptor, encoder, 
//         relin_keys, context, scale
//     );
...
///123 
        ...
        evaluator.relinearize_inplace(C_Leaf_Output, relin_keys); 
        evaluator.rescale_to_next_inplace(C_Leaf_Output); 
        return C_Leaf_Output;
    }
...
void calculate_accuracy(
    const vector<double>& decoded_predictions, // nhãn dự đoán giải mã 
    const vector<vector<double>>& Y_test_onehot, // nhãn thực tế dạng one-hot (dạng vector kích thước NUM_SAMPLES x num_labels)
    size_t NUM_SAMPLES,
    size_t num_labels) 
{
...
int main()
{
    ...
    for (size_t i = 0; i < NUM_SAMPLES_TEST; ++i) {
        ...
        double score_versicolor = decoded_predictions[i + NUM_SAMPLES_TEST]; // Lấy giá trị cột tiếp theo
        double score_virginica = decoded_predictions[i + 2 * NUM_SAMPLES_TEST]; // Lấy giá trị cột thứ 3
