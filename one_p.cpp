// Làm dạng thường để đánh giá model 
...
// change 
size_t number_threshold = 2; // Số ngưỡng để thử nghiệm trong train_decision_tree() thực tế số ngưỡng N x k = 70 x 4 = 280 
size_t max_depth = 2;
...
// Ham danh gia ham soft step dang da thuc dùng cho vector x 
// Input: vector<double> input_values // vector x values , vector<double> coefficients // he so da thuc
// Output: vector<double> output_values // vector y values
vector<double> soft_step_evaluation_old(
    ...
    for (double z : input_values) {
        double result = 0.0; 

        for (int i = 0; i < coefficients.size(); ++i) {            
            result += coefficients[i] * pow(z, i);
        }
...
struct node{
    ...
    // is_leaf == true
    vector<double> leaf_value_vector; // neu la node la thi luu vector trong so cua tung nhan, nếu lưu giá trị thì không biết là label nào
...
vector<double> leaf_value(
    ...
    vector<double> weight_sums(Y_cols.size(), 0.0); // Kích thước L (số nhãn)
    for(int i=0 ; i< Y_cols.size() ; ++i){
        double weight = 0.0;
        for(int j=0 ; j< number_W ; ++j){
            weight += W_col[j] * Y_cols[i][j];
        }
...
// out vecto kich thuoc 1xL 
vector<double> compute_weighted_counts_homo(
    double theta,
    ...
    vector<double> out(Y_cols.size(), 0.0);
    for(int i=0 ; i< Y_cols.size() ; ++i){
        ...
        for(int j=0; j<X_col.size(); ++j){
            out[i] += W_col[j] * soft_step_values[j] * Y_col[j];
        }
    }
    return out;
}

// Tinh gini impurity cho node voi 1 i, 1 nguong 
// Neu compute_gini_impurity<gini_cha thi cap nhat 
...
unique_ptr<node> train_decision_tree(
    ...
    if(depth >= max_depth){
        ...
        node_c->is_leaf = true;
        vector<double> leaf_values = leaf_value(W_col, Y_cols);
        // in ra leaf values
        cout << "\tLeaf values: ";
        for(double val : leaf_values){
            cout << val << " ";
        }
        cout << endl;
    ...
    for(int i=0 ; i<num_feature ; ++i){
        for(int j=0 ; j<number_threshold ; ++j){
            ...
            vector<double> right_counts = compute_weighted_counts_homo(
                theta, X_cols[i], input_values_right, SOFT_STEP_COEFFICIENTS_16, W_col, Y_cols);
            // in ra right_counts
            cout << "\t\tRight counts: ";
            for(double val : right_counts){
                cout << val << " ";
            }
            cout << endl;
            vector<double> left_counts = compute_weighted_counts_homo(
                theta, X_cols[i], input_values_left, SOFT_STEP_COEFFICIENTS_16, W_col, Y_cols);
            cout << "\t\tLeft counts: ";
            for(double val : left_counts){
                cout << val << " ";
            }
            cout << endl;
...
vector<double> predict_decision_tree(
...
)
{
    ...
    if (node_c->is_leaf) {
        ...
        vector<double> Leaf_Output = node_c->leaf_value_vector; 
        return Leaf_Output;
    }
    ...
    // GỌI ĐỆ QUY
    vector<double> Output_Right = predict_decision_tree(
        node_c->right_child, X_cols, num_feature
    );

    vector<double> Output_Left = predict_decision_tree(
        node_c->left_child, X_cols, num_feature
    );

    // KET QUA 
    vector<double> Output_Final(Output_Right.size(), 0.0);
    for(int l=0 ; l< Output_Right.size() ; ++l){
        Output_Final[l] = Phi_Right * Output_Right[l] + Phi_Left * Output_Left[l];
    }
...
void print_tree(const unique_ptr<node>& node_c, int depth = 0) {
    ...
    if (node_c->is_leaf) {
        ...
        for (size_t i = 0; i < node_c->leaf_value_vector.size(); ++i) {
            cout << node_c->leaf_value_vector[i];
...
int main(){
    try{
        ...
        for(int i=0 ; i< NUM_SAMPLES_TEST ; ++i){
            ...
            vector<double> pred = predict_decision_tree(
                model_tree,
                X_sample,
                num_feature
            );
            for(int l=0 ; l< num_label ; ++l){
                decoded_predictions[l][i] = pred[l];
            }
        }
