// Nhân ra hàm dự đoán sai > Sửa lại cách dự đoán 
// Đổi sang dùng vecto<cipher> cho hàm dự đoán 
// Viết lại hàm tính độ chính xác calculate_accuracy()
...
int max_depth = 4;
// int NUM_SAMPLES_TRAIN = 120; // Số mẫu train trong tập dữ liệu iris.csv
// int NUM_SAMPLES_TEST = 30;
...
unique_ptr<Node> train_decision_tree(
    ...
    if (depth >= max_depth) {
        ...
        for (size_t l = 0; l < NUM_LABELS; ++l) {
            ...
            Ciphertext C_weighted_sum = leaf_value(
                C_W_col, C_Y_cols[l], evaluator, relin_keys, gal_keys, context, NUM_SAMPLES 
            );
    ...
    for (int i = 0; i < NUM_FEATURES; ++i) { // i < NUM_FEATURES
        ...
        for (double threshold : all_thresholds) { // 163 ngưỡng // double threshold : all_thresholds
            ...
            auto [C_right_counts, C_left_counts] = compute_weighted_counts_homo(
                ...
                evaluator, encryptor, encoder, relin_keys, gal_keys, context, scale, NUM_SAMPLES
            );
    ...
    node->right_child = train_decision_tree(
        ...
        relin_keys, gal_keys, context, scale, NUM_SAMPLES
    );
...
)
{
    ...
    if (node->is_leaf) {
        ...
        for (size_t l = 0; l < NUM_LABELS; ++l) {
            ...
            vector<double> leaf_expanded(encoder.slot_count());
            for (size_t s = 0; s < encoder.slot_count(); ++s) leaf_expanded[s] = node->leaf_value[l];
...
)
{
    ...
    double acc = (double)correct / (double)NUM_SAMPLES;
...
)
{
    ...
    const size_t N = decoded_predictions[0].size();
...
int main()
{
    ...
    unique_ptr<Node> root = train_decision_tree(
        ...
        evaluator, encryptor, decryptor, encoder, relin_keys, gal_keys, context, scale, NUM_SAMPLES
    );
